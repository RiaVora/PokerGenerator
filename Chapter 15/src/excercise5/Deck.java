...
//	    
	    ...
	    //mergeSort() as a class method
	    public static Deck mergeSort(Deck d) {
			if (d.cards.length < 2) {
			    return d;
			}
			int mid = (d.cards.length-1) / 2;
			
			// divide the deck roughly in half
			Deck d1 = d.subdeck(0, mid);
			Deck d2 = d.subdeck(mid+1, d.cards.length-1);
